...
class PlayStateMobile extends FlxState
{
    var cat:FlxSprite;
	...
	{
		begen = false;
		ded.text = 'Touch anywhere to begin';
		ded.screenCenter(X);
        ded.screenCenter(Y);
	} 
	...
	override public function update(elapsed:Float)
	{
		...
		boltGen.x = FlxG.random.int(0, 1560);
		...
		if(FlxG.random.bool(5) && bolt10.y >=800)
		{
			if (!FlxG.save.data.sistemFullPow)
			{
				bolt10.revive();
				FlxTween.tween(bolt10, {alpha: 1}, 0.08, {ease: FlxEase.quartInOut});
				bolt10.x = boltGen.x;
				bolt10.y = 0;
			}
			else if (FlxG.save.data.sistemFullPow)
		...
		if(FlxG.random.bool(5) && bolt11.y >=800)
		{
			if (!FlxG.save.data.sistemFullPow)
			{
				bolt11.revive();
				FlxTween.tween(bolt11, {alpha: 1}, 0.08, {ease: FlxEase.quartInOut});
				bolt11.x = boltGen.x;
				bolt11.y = 0;
			}
			else if (FlxG.save.data.sistemFullPow)
		...
		if(FlxG.random.bool(5) && bolt12.y >=800)
		{
			if (!FlxG.save.data.sistemFullPow)
			{
				bolt12.revive();
				FlxTween.tween(bolt12, {alpha: 1}, 0.08, {ease: FlxEase.quartInOut});
				bolt12.x = boltGen.x;
				bolt12.y = 0;
			}
			else if (FlxG.save.data.sistemFullPow)
		...
		if(FlxG.random.bool(5) && bolt.y <=-200)
			...
			if(FlxG.random.bool(5) && bolt10.y <=-200)
			{
				if (!FlxG.save.data.sistemFullPow)
				{
					bolt10.revive();
			...
			if(FlxG.random.bool(5) && bolt11.y <=-200)
			{
				if (!FlxG.save.data.sistemFullPow)
				{
					bolt11.revive();
			...
			if(FlxG.random.bool(5) && bolt12.y <=-200)
			{
				if (!FlxG.save.data.sistemFullPow)
				{
					bolt12.revive();
		...
		if(begen) //&& !FlxG.save.data.sistemMaso)
		{
			for (touch in FlxG.touches.list)
			{
				...
				{
                    if (cat.x < touch.x - 152 || cat.x > touch.x + 152) dummy++;         //The dummy int is not used for the mechanic, just for filling
					else cat.x = touch.x - 52;
					if (cat.y < touch.y - 156 || cat.y > touch.y + 156) dummy++;
		...
		{
		    if(cat.overlaps(bolt) || cat.overlaps(bolt1) || cat.overlaps(bolt2) || cat.overlaps(bolt3) || cat.overlaps(bolt4) || cat.overlaps(bolt5) || cat.overlaps(bolt6) || cat.overlaps(bolt7) || cat.overlaps(bolt8) || cat.overlaps(bolt9) || cat.overlaps(bolt10) || cat.overlaps(bolt11) || cat.overlaps(bolt12))
		    {
			    IFrame = true;
				cat.color = 0xfa1d1d;
		...
		if(!isAlive && begen)
		{
            ded.text = 'You died. Touch anywhere to play again';
			ded.screenCenter(X);
			ded.screenCenter(Y);
		...
		if (!FlxG.save.data.sistemInvert)
		{
			...
			if(bolt10.y >= 800) {bolt10.kill(); bolt10.alpha = 0;}
		    if(bolt11.y >= 800) {bolt11.kill(); bolt11.alpha = 0;}
		    if(bolt12.y >= 800) {bolt12.kill(); bolt12.alpha = 0;}
		}
		else if (FlxG.save.data.sistemInvert)
		{
			...
			if(bolt10.y <=-200) {bolt10.kill(); bolt10.alpha = 0;}
		    if(bolt11.y <=-200) {bolt11.kill(); bolt11.alpha = 0;}
		    if(bolt12.y <=-200) {bolt12.kill(); bolt12.alpha = 0;}
		}
